...
/*-----------------------------COLORS------------------------------------*/
		/*Header Color*/
		<?php
    		$headerBGCss =  $optimizer['head_color_id'];
    		if(!empty($optimizer['head_nav_borderbottom_image_url']))
	        {
	            $headerBGCss = 'url(' . $optimizer['head_nav_borderbottom_image_url'] .') bottom center repeat-x ' . $headerBGCss;
    		}
		?>
		.header{ position:relative!important; background:<?php echo $headerBGCss; ?>; background-size:contain;}
...
<?php if($optimizer['sec_color_id']){ ?>
		/*BASE Color*/
		.heading_border, #wp-calendar #today, .thn_post_wrap .more-link:hover, .moretag:hover, .search_term #searchsubmit, .error_msg #searchsubmit, #searchsubmit, .optimizer_pagenav a:hover, .nav-box a:hover .left_arro, .nav-box a:hover .right_arro, .pace .pace-progress, .homeposts_title .menu_border, .pad_menutitle, .ast_login_widget #loginform #wp-submit, .prog_wrap, .lts_layout1 a.image, .lts_layout2 a.image, .lts_layout3 a.image, .rel_tab:hover .related_img, .wpcf7-submit, .woo-slider #post_slider li.sale .woo_sale, .nivoinner .slide_button_wrap .lts_button, #accordion .slide_button_wrap .lts_button, .img_hover, p.form-submit #submit, .optimposts .type-product a.button.add_to_cart_button{background:<?php echo $optimizer['sec_color_id'] ?>;} 
...
<?php
    $footerBGCss =  $optimizer['footer_color_id'];
    if(!empty($optimizer['footer_background_image_url']))
    {
        $footerBGCss = 'url(' . $optimizer['footer_background_image_url'] .') center center repeat ' . $footerBGCss;
    }
?>
...
#footer{background: <?php echo $footerBGCss ?>;}
...
@media screen and (max-width: 960px)
{
    <?php
    	$headerBGCss =  $optimizer['head_color_id'];
    	if(!empty($optimizer['head_nav_borderbottom_min_image_url']))
        {
            $headerBGCss = 'url(' . $optimizer['head_nav_borderbottom_min_image_url'] .') bottom center repeat-x ' . $headerBGCss;
    	}
    ?>
    .header{ background:<?php echo $headerBGCss; ?>;}
}
